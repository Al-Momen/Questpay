...
<section>
    <div class="container">
        <div class="document-header d-flex flex-wrap justify-content-between align-items-center mb-2">
            <div class="logo"><a href="{{ route('home') }}"><img src="{{ siteLogo() }}"
                        alt=""></a></div>
            <nav class="navbar navbar-expand-lg navbar-light">
                <div class="collapse navbar-collapse" id="navbarNavDropdown">
                    <ul class="navbar-nav ms-auto">
                        @php
                            $pages = App\Models\Menu::with(['items', 'menuItems'])->where('code','header_menu')->first();
                        @endphp
                        ...
                        @foreach($pages->items as $k => $data)
                            ...
                            @else
                                <li class="nav-item {{ route('pages',[$data->url]) == url()->current() ? 'active' : null }}">
                                    <a href="{{route('pages',[$data->url])}}" class="nav-link">{{__($data->title)}}</a>
                                </li>
                            @endif
                        @endforeach
